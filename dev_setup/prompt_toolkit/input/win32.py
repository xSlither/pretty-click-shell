...
class ConsoleInputReader(object):
    ...
    def _handle_mouse(self, ev):
        ...
        # Check event type.
        if ev.ButtonState in [ FROM_LEFT_1ST_BUTTON_PRESSED, FROM_LEFT_2ND_BUTTON_PRESSED] :
            ...
            for event_type in [MouseEventType.MOUSE_DOWN, MouseEventType.MOUSE_UP]:
                try:
                    data = ";".join(
                        [event_type.value, str(ev.MousePosition.X), str(ev.MousePosition.Y)] if not isinstance(event_type, str) else [event_type, str(ev.MousePosition.X), str(ev.MousePosition.Y)]
                    )
